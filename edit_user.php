...
if(isset($_POST['update'])){
    ...
    // =======================
    // KIRIM PESAN WELCOME JIKA TELEGRAM CHAT ID BARU DITAMBAHKAN
    // =======================
    if ($update && !empty($telegram_chat_id) && $telegram_chat_id != $user['telegram_chat_id']) {
        require_once 'config/telegram.php';
        if (function_exists('kirimTelegram')) {
            $pesan_tg = "🎉 <b>Halo " . htmlspecialchars($full_name) . "!</b>\n\n";
            $pesan_tg .= "ID Telegram Anda berhasil dihubungkan ke akun SI-LIAK Anda.\n";
            $pesan_tg .= "Mulai sekarang, Anda akan menerima berbagai notifikasi penting (seperti update request, pesan baru, dll) langsung di sini.";
            kirimTelegram($telegram_chat_id, $pesan_tg);
        }
    }
